...
def main():
    ...
    if args.evaluate:
        validate(val_loaders, model)
        return

    criterion = build_criterion(config.criterion)

    for task_id, (train_loader_task, val_loader_task) in enumerate(zip(train_loaders, val_loaders)):
...
def validate(val_loader, model, criterion, task_id=None):
